...
class ThreeDChangeDetection:
    ...
    def total_error(self, old_error, new_error):

        old_input = old_error

        new_input = new_error

        old_entry = QgsRasterCalculatorEntry()
        old_entry.raster = old_input
        old_entry.bandNumber = 1
        old_entry.ref = 'olderror@1'

        new_entry = QgsRasterCalculatorEntry()
        new_entry.raster = new_input
        new_entry.bandNumber = 1
        new_entry.ref = 'newerror@1'
        ...
        outRaster = QgsProcessingUtils.generateTempFilename('total_error.tif')

        change = QgsRasterCalculator('%s + %s' % (old_entry.ref, new_entry.ref), outRaster, "GTiff", e, w, h,
                                     entries)
        change.processCalculation()

        output = QgsRasterLayer(outRaster, "total error")
